# BPI Baseline 
# Module E Other income

setwd("C:/Users/dan.killian/Dropbox/work/MSI/Chevron Bangladesh (BPI)/survey/BPI Enterprise Development Baseline Report/Data")
setwd("C:/Users/Dan/Dropbox/work/MSI/Chevron Bangladesh (BPI)/survey/BPI Enterprise Development Baseline Report/Data")

?read.spss
# foreign
dat <- read.spss("Raw Data-file (BPI).sav", to.data.frame=T)
dat <- read.spss("Final Datafile (BPI).sav", to.data.frame=T)
# haven
dat <- read_spss("Raw Data-file (BPI).sav")
dat <- read_spss("Final Datafile (BPI).sav")


datcsv <- read.csv("Raw Data-file (BPI).csv")
dat <- read.csv("Final Datafile (BPI).csv")

datNames <- codebook(dat)

# correct for errors in a20
which(dat[,1]!=dat$a20)
dat$a20 <- dat[,1]

modE <- dplyr::select(dat, hid=a20, dist:res_type2, ec1:e2_56)
modENames <- codebook(modE)
str(modE)

modE[is.na(modE)] <- 0

write.csv(modE, "modE.csv", row.names=F)

modE <- read.csv("modE.csv")
modEnames <- codebook(modE)
...
assets$Score <- round(a$scores,2)
attributes(assets$Score) <- NULL
describe(assets$Score)
str(assets$Score)
